    ...
    <nav className="fixed inset-x-0 top-0 z-50">
      ...
      >
        ...
        >
          ...
          <span className="relative z-30 block h-5 w-6">
            <span
              className={`absolute left-0 top-0 block z-30 h-[2px] w-6 rounded bg-white transition-transform duration-300
                         ${isOpen ? "translate-y-[9px] rotate-45" : ""}`}
            />
            <span
              className={`absolute left-0 top-1/2 block z-30 h-[2px] w-6 -translate-y-1/2 rounded bg-white transition-opacity duration-300
                         ${isOpen ? "opacity-0" : "opacity-100"}`}
            />
            <span
              className={`absolute left-0 bottom-0 block z-30 h-[2px] w-6 rounded bg-white transition-transform duration-300
                         ${isOpen ? "-translate-y-[9px] -rotate-45" : ""}`}
            />
          </span>
        </button>
      ...
      >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="bg-white cursor-pointer py-4 px-16 text-black md:px-20 tracking-[-6%] md:py-8 rounded-full font-medium text-[14px]
                     hover:bg-purple-100 transition-colors inline-flex items-center gap-2"
        >
          <span>Voir mon CV</span>
          <Image src={Open} alt="Ouvrir" width={16} height={16} />
